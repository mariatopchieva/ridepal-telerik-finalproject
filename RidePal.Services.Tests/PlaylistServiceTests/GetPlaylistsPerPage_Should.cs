...
{
    [TestClass]
    public class GetPlaylistsPerPage_Should
    {
        [TestMethod]
        public void ReturnTwoPlaylistsOnFirstPage()
        {
            //Arrange
            var options = Utils.GetOptions(nameof(ReturnTwoPlaylistsOnFirstPage));
            ...
            using (var assertContext = new RidePalDbContext(options))
            {
                ...
                var result = sut.GetPlaylistsPerPage(1).ToList();
